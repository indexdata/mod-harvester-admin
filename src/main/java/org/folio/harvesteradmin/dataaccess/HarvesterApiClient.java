...
public class HarvesterApiClient {
  ...
  protected static final Logger logger = LogManager.getLogger(HarvesterApiClient.class);
  ...
  static {
    ...
    rootOfResultSetByHarvesterPath.put(HARVESTER_TSAS_PATH,
  ...
  /**
   * Gets supported request parameters.
   */
  ...
  private static String buildQueryString(Map<String, String> parameterMap) {
    ...
    if (parameterMap != null) {
      ...
      for (int i = 0; i < keys.length; i++) {
        ...
        queryString.append("=");
        queryString.append(URLEncoder.encode(parameterMap.get(key), StandardCharsets.UTF_8));
      }
    }
      ...
      getConfigRecordById(harvesterPath, id, tenant).onComplete(getById -> {
        if (getById.result().wasOK()) {
          ...
          if (getById.result().wasInternalServerError()) {
            logger.error(
                " GET by ID (" + id + ") to " + harvesterPath + " encountered a server error: "
      ...
      if (idLookup.succeeded()) {
        ...
        if (idLookup.result().wasNotFound()) {
          ...
          harvesterGetRequest(HARVESTER_HARVESTABLES_PATH + "/" + id + "/log").send(
              ar -> responseText(routingContext, 200).end(ar.result().bodyAsString()));
        } else {
          responseText(routingContext, idLookUpResponse.statusCode()).end(
              "There was an error (" + idLookUpResponse.statusCode() + ") looking up "
                  + HARVESTER_HARVESTABLES_PATH + "/" + id + " to get logs: "
                  + idLookUpResponse.errorMessage());
        }
      } else {
        responseText(routingContext, INTERNAL_SERVER_ERROR).end(
            "Could not look up harvest job " + HARVESTER_HARVESTABLES_PATH + "/" + id
                + " to get logs : " + idLookup.cause().getMessage());
      }
    });
  }
   ...
   * Gets config record.
   */
  protected Future<ProcessedHarvesterResponseGetById> getConfigRecordById(String harvesterPath,
                                                                          String id,
                                                                          String tenant) {
  ...
  /**
   * Puts config record and responds.
   */
                      ...
                      put -> {
                        if (put.succeeded()) {
                          if (put.result().statusCode() == NO_CONTENT) {
                            ...
                            responseText(routingContext, put.result().statusCode()).end(
                                "There was a problem PUTting to " + harvesterPath + "/" + id + ": "
                                    + put.result().statusMessage());
                          }
                        } else {
                          responseText(routingContext, INTERNAL_SERVER_ERROR).end(
   ...
   * Puts config record.
   */
  protected Future<HttpResponse<Buffer>> putConfigRecord(RoutingContext routingContext,
                                                         JsonObject jsonToPut, String generatedId,
                                                         String harvesterPath, String tenant) {
  ...
  /**
   * Validates, POSTs, and returns a new harvest configuration record.<br/> <br/> This method
   * proxies the
   * corresponding legacy Harvester end-point to make this API compliant with the conventions of
   * FOLIO APIs (and
   * indeed most REST APIs)<br/> <br/> The legacy API will not respond conventionally when
   * receiving a POST request
   * for an already existing entity, and it will also not return a newly created entity to the
   * caller as e.g. FOLIO
   * APIs do.<br/> <br/> To normalize the REST behavior of POSTs, this method will thus first
   * look up the entity by
   * ID, if an ID is supplied, and return a 422 (unprocessable) if an entity with that ID exists
   * already, otherwise it
   * will go ahead and POST the entity. Next, if the POST is successful, the method will retrieve
  ...
  public void postConfigRecordAndRespond(RoutingContext routingContext, String harvesterPath,
                                         String tenant) {
    if (harvesterPath.equals(HARVESTER_TSAS_PATH)) {
                ...
                if (idLookUp.succeeded()) {
                  if (idLookUp.result().wasOK()) {
                    ...
                    responseText(routingContext, idLookUp.result().statusCode()).end(
                        "There was an error (" + idLookUp.result().statusCode() + ") looking up "
                            + harvesterPath + "/" + id + " before POST: " + idLookUp.result()
                            .errorMessage());
                  }
                } else {
                  responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                      "Could not look up record " + harvesterPath + "/" + id + " before POST: "
                          + idLookUp.cause().getMessage());
                }
              });
        }
  ...
  /**
   * Method POSTs a Harvester configuration record and sends a response containing the newly
   * created record if
   ...
   */
  private void doPostAndRetrieveAndRespond(RoutingContext routingContext, JsonObject jsonToPost,
                                           String harvesterPath, String tenant) {
    doPostAndRetrieve(jsonToPost, harvesterPath, tenant).onComplete(postAndRetrieve -> {
      ProcessedHarvesterResponsePost response =
          (ProcessedHarvesterResponsePost) postAndRetrieve.result();
      if (response.wasCreated()) {
        responseJson(
            routingContext, response.statusCode()).putHeader("Location", response.location)
            ...
            idLookUp -> { // going to return 422 if found
              if (idLookUp.succeeded()) {
                if (idLookUp.result().wasOK()) {
                  ...
                  responseText(routingContext, idLookUp.result().statusCode()).end(
                      "There was an error (" + idLookUp.result().statusCode() + ") looking up "
                          + HARVESTER_TRANSFORMATIONS_PATH + "/" + id + " before POST: "
                          + idLookUp.result().errorMessage());
                }
              } else {
                responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                    "Could not look up record " + HARVESTER_TRANSFORMATIONS_PATH + "/" + id
                        + " before POST: " + idLookUp.cause().getMessage());
              }
            });
      } else {
  ...
  private Future<ProcessedHarvesterResponse> doPostAndRetrieve(JsonObject jsonToPost,
        ...
        if (ar.succeeded()) {
          ...
          if (ar.result().statusCode() == CREATED && location != null) {
            ...
            lookUpHarvesterRecordById(harvesterPath, idFromLocation, tenant).onComplete(
                // going to return 500, internal server error if not found, 201, Created if
                // found
                lookUpNewlyCreatedRecord -> promise.complete(
                    new ProcessedHarvesterResponsePost(ar, harvesterPath, harvesterPath,
                        lookUpNewlyCreatedRecord.result())));
          } else {
            promise.complete(
                new ProcessedHarvesterResponsePost(ar, harvesterPath, harvesterPath, null));
          }
        } else {
          promise.complete(
              new ProcessedHarvesterResponsePost(ar, harvesterPath, harvesterPath, null));
  ...
  private void postTsaAndRespond(RoutingContext routingContext, String tenant) {
    ...
    if (isNonJsonContentType(routingContext)) {
      ...
      String id = requestJson.getString("id");
      if (id != null) {
        lookUpHarvesterRecordById(HARVESTER_TSAS_PATH, id, tenant).onComplete(
            idLookUp -> { // going to return 422 if found
              if (idLookUp.succeeded()) {
                if (idLookUp.result().wasOK()) {
                  responseText(routingContext, 422).end(
                      HARVESTER_TSAS_PATH + "/" + id + " already exists");
                } else if (idLookUp.result().wasNotFound()) {
                  doPostTsaPutTransformationAndRespond(routingContext, tenant);
                } else {
                  responseText(routingContext, idLookUp.result().statusCode()).end(
                      "There was an error (" + idLookUp.result().statusCode() + ") looking up "
                          + HARVESTER_TSAS_PATH + "/" + id + " before POST: "
                          + idLookUp.result().errorMessage());
                }
              } else {
                responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                    "Could not look up record " + HARVESTER_TSAS_PATH + "/" + id
                        + " before POST: " + idLookUp.cause().getMessage());
              }
            });
      } else {
              ...
              if (transformationPost.succeeded()
                ...
                for (int i = 0; i < result.result().size(); i++) {
                  ...
                  tsaJson.getJsonObject("step")
                      .put("entityType", typeToEmbeddedTypeMap.get(stepJson.getString("type")));
                      ...
                      if (putResponse.succeeded()) {
                        ...
                        responseText(routingContext, BAD_REQUEST).end(
                            " Failed to POST (with subsequent PUT and GET) of Transformation."
                                + " PUT failed with  " + putResponse.result().statusMessage());
                      }
                    });
              }
            });
    ...
    lookUpHarvesterRecordById(HARVESTER_TRANSFORMATIONS_PATH, transformationId, tenant).onComplete(
        checkTransformation -> {
          if (!checkTransformation.result().found()) {
            ...
            lookUpHarvesterRecordById(HARVESTER_STEPS_PATH, stepId, tenant).onComplete(
                checkStep -> {
                    if (!checkStep.result().found()) {
                      ...
                      doPostAndRetrieve(incomingTsa, HARVESTER_TSAS_PATH,
                                          ...
                                          try {
                                            ...
                                            String xml =
                                                JsonToHarvesterXml.convertToHarvesterRecord(
                                                    ...
                                                    EntityRootNames.TRANSFORMATION_ROOT_PROPERTY,
                                                    tenant);
                                            harvesterPutRequest(HARVESTER_TRANSFORMATIONS_PATH
                                                + "/"
  ...
  /**
   * Deletes a config record and responds.
   */
        ...
        idLookUp -> {  // going to return 404 if not found, else 204 (no content/deleted)
          if (idLookUp.succeeded()) {
            ...
            if (idLookUp.result().wasNotFound()) {
              ...
              harvesterDeleteRequest(harvesterPath + "/" + id).send(ar -> {
                if (ar.succeeded()) {
                  if (ar.result().statusCode() == NO_CONTENT) {
                    ...
                    responseText(routingContext, ar.result().statusCode()).end(
                        "Could not delete " + harvesterPath + "/" + id + ": " + ar.result()
                            .bodyAsString());
                  }
                } else {
                  responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                      "There was an error deleting "
                          + harvesterPath + "/" + id + ": " + ar.cause()
                          .getMessage());
                }
              });
            } else {
              responseText(routingContext, idLookUp.result().statusCode()).end(
                  "There was an error (" + idLookUp.result().statusCode() + ") looking up "
                      + harvesterPath + "/" + id + " before DELETE: " + idLookUp.result()
                      .errorMessage());
            }
          } else {
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
  ...
  private Future<AsyncResult<HttpResponse<Buffer>>> deleteConfigRecord(
      RoutingContext routingContext, String harvesterPath, String id, String tenant) {
  ...
  /**
   * Gets a config record and responds.
   */
    ...
    if (isNonJsonContentType(routingContext)) {
      ...
      getConfigRecords(harvesterPath, queryParameters, tenant).onComplete(get -> {
        if (get.result().wasOK()) {
          ...
          logger.error("GET " + harvesterPath + (query.isEmpty() ? "" : "(" + query + ")")
              + " encountered a server error: " + get.result().errorMessage());
  ...
  private Future<Boolean> noHarvestablesFound(String tenant) {
    ...
    restClient
        .get(
            LegacyServiceConfig.harvesterPort,
            LegacyServiceConfig.harvesterHost,
            HARVESTER_HARVESTABLES_PATH)
        .send(ar -> {
          ProcessedHarvesterResponseGet response =
              new ProcessedHarvesterResponseGet(
                  ar, HARVESTER_HARVESTABLES_PATH, "acl=" + tenant);
          if (response.wasOK()) {
  ...
  private Future<Boolean> noStepAssociationsFound(String tenant) {
    ...
    restClient
        ...
        .send(ar -> {
          ProcessedHarvesterResponseGet response =
              new ProcessedHarvesterResponseGet(ar, HARVESTER_TSAS_PATH, null);
          if (response.wasOK()) {
            int records = Integer.parseInt(response.jsonObject().getString("totalRecords"));
            promise.complete(records == 0);
          } else {
            logger.error("There was a problem looking up transformation-step associations (tsas): "
  ...
  private Future<Boolean> noJobsRunning(RoutingContext routingContext, String tenant) {
    ...
    Promise<Boolean> promise = Promise.promise();
    restClient.get(LegacyServiceConfig.harvesterPort, LegacyServiceConfig.harvesterHost,
        HARVESTER_HARVESTABLES_PATH + "?" + query).send(ar -> {
          ProcessedHarvesterResponseGet response =
              new ProcessedHarvesterResponseGet(ar, HARVESTER_HARVESTABLES_PATH, query);
          if (response.wasOK()) {
            int records = Integer.parseInt(response.jsonObject().getString("totalRecords"));
            promise.complete(records == 0);
          } else {
  ...
  protected Future<ProcessedHarvesterResponseGetById> lookUpHarvesterRecordById(
      String harvesterPath, String id, String tenant) {
    Promise<ProcessedHarvesterResponseGetById> promise = Promise.promise();
    harvesterGetRequest(harvesterPath + "/" + id).send(ar -> {
      ProcessedHarvesterResponseGetById adaptedResponse =
          new ProcessedHarvesterResponseGetById(ar, harvesterPath, id, tenant);
   ...
   * Gets config records.
   */
  public Future<ProcessedHarvesterResponseGet> getConfigRecords(String harvesterPath,
                                                                Map<String, String> queryParameters,
                                                                String tenant) {
    ...
    String pathAndQuery =
        harvesterPath + (query.isEmpty() ? aclFilter(tenant) : query + andAclFilter(tenant));
      ...
      case HARVESTER_HARVESTABLES_PATH:
        noJobsRunning(routingContext, tenant).onComplete(result -> {
          if (result.succeeded()) {
            ...
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                "There was a problem checking for running jobs, deletion of harvestables "
                    + "aborted: " + result.cause().getMessage());
          }
        });
        break;
        ...
        noHarvestablesFound(tenant).onComplete(noHarvestables -> {
          if (noHarvestables.succeeded()) {
            if (noHarvestables.result()) {
              ...
              responseText(routingContext, BAD_REQUEST).end(
                  "Cannot delete all transformation pipelines, "
                      + "there are harvestable(s) potentially using some");
            }
          } else {
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                "There was a problem checking for dependent harvestables, "
                    + "deletion of transformations aborted: " + noHarvestables.cause()
                    .getMessage());
          }
        });
        break;
      case HARVESTER_STEPS_PATH:
        noStepAssociationsFound(tenant).onComplete(noAssociations -> {
          if (noAssociations.succeeded()) {
            ...
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                "There was a problem checking for step associations, deletion of steps aborted: "
                    + noAssociations.cause().getMessage());
          }
        });
        break;
      case HARVESTER_STORAGES_PATH:
        noHarvestablesFound(tenant).onComplete(noHarvestables -> {
          if (noHarvestables.succeeded()) {
            ...
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                "There was a problem checking for dependent harvestables, "
                    + "deletion of storages aborted: " + noHarvestables.cause().getMessage());
          }
        });
        break;
      case HARVESTER_TSAS_PATH:
        noJobsRunning(routingContext, tenant).onComplete(result -> {
          if (result.succeeded()) {
            if (result.result()) {
            ...
            responseText(routingContext, INTERNAL_SERVER_ERROR).end(
                "There was a problem checking for running jobs, "
                    + "deletion of transformation step associations aborted: " + result.cause()
                    .getMessage());
          }
        });
        break;
  ...
  private void deleteAllRecordsAndRespond(RoutingContext routingContext, String harvesterPath,
                                          String tenant) {
        ...
        if (get.result().totalRecords() > 0) {
          ...
          for (Object o : get.result().getRecords()) {
            ...
            deleteEntityFutures.add(
                deleteConfigRecord(
                    routingContext, harvesterPath, entity.getString("id"), tenant));
          }
          CompositeFuture.all(deleteEntityFutures).onComplete(result -> {
            if (result.succeeded()) {
              ...
              responseText(routingContext, 500).end(
                  "There was a problem deleting records at " + harvesterPath + ": "
                      + result.cause());
            }
          });
        }
